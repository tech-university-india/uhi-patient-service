...
describe('Patient Service', () => {
  describe('checkIfPatientExists', () => {
    it('should return "Patient exists" if patient exists', async () => {
      jest.spyOn(db.Patient, 'findOne').mockResolvedValueOnce('Patient exists' as any)
    ...
    it('should throw an error if patient does not exist', async () => {
      ...
      await expect(patientService.checkIfPatientExists('123')).rejects.toThrow('Patient does not exist')
    })
  })
  ...
  describe('createPatient', () => {
    it('should create a patient', async () => {
      jest.spyOn(db.Patient, 'findOne').mockResolvedValueOnce(null as any)
      jest.spyOn(db.Patient, 'create').mockResolvedValueOnce('Patient Created Successfully' as any)
  ...
  describe('getPatients', () => {
    it('should return all patients', async () => {
      jest.spyOn(db.Patient, 'findAll').mockResolvedValueOnce(['Patient 1', 'Patient 2'] as any)
  ...
  describe('getPatient', () => {
    ...
    it('should throw an error if patient does not exist', async () => {
      jest.spyOn(db.Patient, 'findOne').mockResolvedValueOnce(null as any)
      await expect(patientService.getPatient('123')).rejects.toThrow('Patient does not exist')
    })
  })
  ...
  describe('updatePatient', () => {
    it('should update a patient', async () => {
      ...
      jest.spyOn(db.Patient, 'findOne').mockResolvedValueOnce('Patient' as any)
      const result = await patientService.updatePatient({ abhaId: '123', name: 'John', gender: 'M', yearOfBirth: 2001, monthOfBirth: 12, dayOfBirth: 7, address: {}, mobile: '8317261525', healthNumber: '12345' })
      expect(result).toEqual('Patient')
    })
  })
  ...
  describe('deletePatient', () => {
    ...
    it('should throw an error if patient does not exist', async () => {
      jest.spyOn(db.Patient, 'findOne').mockResolvedValueOnce(null as any)
      await expect(patientService.deletePatient('123')).rejects.toThrow('Patient does not exist')
    })
  })
})