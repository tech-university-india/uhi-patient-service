...
export const createPatient = async (
  ...
  JSON.parse(JSON.stringify(patientInfo));
  let {
    abhaId,
    name,
    gender,
  ...
  if (healthNumber.indexOf('-') === -1) {
    healthNumber = `${healthNumber.slice(0, 2)}-${healthNumber.slice(
      2,
      6
    )}-${healthNumber.slice(6, 10)}-${healthNumber.slice(10, 14)}`;
  }
...
export const getPatient = async (
  abhaId: string
): Promise<{data: PatientInstance; message: string}> => {
  const patient = await db.Patient.findOne({where: {abhaId}});
  if (patient) {
    return {
      data: patient,
